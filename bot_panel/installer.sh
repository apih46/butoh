...
surat=$(curl -sS https://raw.githubusercontent.com/apih46/butoh/main/authbot | grep -w $ipes | awk '{print $4}'  )
if [[ "$surat" = "true" ]]; then
echo -ne
else
...
dircreate
        ...
        echo -e "lukavpn Bot Panel Installer
        "
        [[ ! -f /root/ResBotAuth ]] && {
        ...
        echo -ne "Username admin panel use '@' [Ex: @lukavpn] : "
        read admin_pnl
        [[ -z $admin_pnl ]] && admin_pnl="@lukavpn"
        echo ""
        echo -ne "Limit trial for reseller create user trial [default:1] : "
        read limit_pnl
        [[ -z $limit_pnl ]] && limit_pnl="1"
        echo ""
        echo -ne "Your name store [dafult: lukavpn-STORE] : "
        read store_pnl
        [[ -z $store_pnl ]] && store_pnl="lukavpn-STORE"
        echo ""
...
EOF
        ...
        fun_bot1() {
			...
			[[ "$(grep -wc "lukavpn_bot" "/etc/rc.local")" = '0' ]] && {
			    sed -i '$ i\screen -dmS lukavpn_bot bbt' /etc/rc.local >/dev/null 2>&1
			}
        }
        screen -dmS lukavpn_bot bbt >/dev/null 2>&1
        fun_bot1
        [[ $(ps x | grep "lukavpn_bot" | grep -v grep | wc -l) != '0' ]] && echo -e "\nBot successfully activated !" || echo -e "\nError1! Information not valid !"
        sleep 2
        menu
        } || {
       clear
        echo -e "Info...\n"
        fun_bot2() {
            screen -r -S "lukavpn_bot" -X quit >/dev/null 2>&1
            [[ $(grep -wc "lukavpn_bot" /etc/rc.local) != '0' ]] && {
                sed -i '/lukavpn_bot/d' /etc/rc.local
            }
