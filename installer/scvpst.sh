...
linktrgo="trojan-go://${uuidR}@${domain}:${trgo}/?sni=${domain}%26type=ws%26host=${domain}%26path=/lukavpn%26encryption=none#${userna}"
        ...
        (
            ...
            if [[ ${message_reply_to_message_message_id[$id]} ]]; then
                ...
                '👤 CREATE USER 👤\n\nUsername:')
                    ...
                    [[ "$sizemax" -gt '10' ]] && {
                        ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
                            --text "$(echo -e "⛔ Use no maximum 10 characters [EX: lukavpn]")" \
                            --parse_mode html
                        >$CAD_ARQ
                        break
                    }
                ...
                'User token by generate:')
                    ...
                    echo "${message_text[$id]}" >/tmp/lukavpnss
