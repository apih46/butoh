...
systemctl enable lukavpnsshws.service >/dev/null 2>&1
systemctl start lukavpnsshws.service >/dev/null 2>&1
...
PID=`ps -ef |grep -v grep | grep sshws |awk '{print $2}'`
if [[ ! -z "${PID}" ]]; then
systemctl stop lukavpnsshws.service
