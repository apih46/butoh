...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/apih46/butoh/main/skkkk > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/apih46/butoh/main/skkkk | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/apih46/butoh/main/skkkk | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
curl -sS "https://raw.githubusercontent.com/apih46/butoh/main/sstp/accel.conf" | openssl aes-256-cbc -d -a -pass pass:lukavpn07gg -pbkdf2 > /etc/accel-ppp.conf
